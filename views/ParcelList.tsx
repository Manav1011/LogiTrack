  ...
  const renderAction = (parcel: Parcel) => {
        ...
        if (parcel.currentStatus === ParcelStatus.BOOKED) {
            return (
                <div className="flex gap-2 justify-end">
                    ...
                    <ActionButton onClick={() => updateParcelStatus(parcel.id, ParcelStatus.IN_TRANSIT)} colorClass="bg-sky-50 text-sky-700 border-sky-200 hover:bg-sky-100" icon={Truck} label="Dispatch" />
                </div>
            );
        }
    ...
    if (parcel.destinationOfficeId === myOfficeId) {
      if (parcel.currentStatus === ParcelStatus.IN_TRANSIT) {
        return <ActionButton onClick={() => updateParcelStatus(parcel.id, ParcelStatus.ARRIVED)} colorClass="bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100" icon={MapPin} label="Arrive" />;
      }
      if (parcel.currentStatus === ParcelStatus.ARRIVED) {
        return <ActionButton onClick={() => updateParcelStatus(parcel.id, ParcelStatus.DELIVERED)} colorClass="bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100" icon={CheckCircle} label="Deliver" />;
      }
    }
             ...
             <tbody className="divide-y divide-slate-100">
                {myParcels.map(parcel => (
                   <tr key={parcel.id} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-6 py-4 align-top">
                         <div className="font-medium text-slate-800">{parcel.quantity}x {parcel.type}</div>
                         <div className="text-xs text-slate-500 mt-1">{parcel.weightKg}kg • ${parcel.price}</div>
                      </td>
                      <td className="px-6 py-4 align-top">
